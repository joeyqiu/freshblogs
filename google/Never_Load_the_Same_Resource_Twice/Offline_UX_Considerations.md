## Offline UX Considerations(Offline UX 注意事项)
...
![Emojoy，表情符号消息应用，当状态发生变化时通知用户。](./images/emojoy-toast-message.png)当状态发生变化时尽快清楚地通知用户。

![I/O 2016 应用，当状态发生变化时通知用户。](./images/io-toast-message.png)Google I/O 应用使用一个 Material Design“toast”通知用户其处于离线状态。

网络连接已改善或已恢复

![一个天气应用示例。](./images/weather-app.png)某些应用，如这个天气应用，需要自动更新，因为旧数据对用户没有任何用。

在用户的网络连接改善后如何通知用户在很大程度上取决于您的应用。 对于需要将显示的更新信息作为优先级的应用，如股市应用，尽快自动更新和通知用户非常重要。

您可能还会始终在醒目位置显示最后一次更新应用的时间。 例如，这对于货币转换应用同样非常有用

![过时的 Material Money 应用](./images/material-money-rates-out-of-date.png)![Material Money 已更新](./images/material-money-rates-updated.png)Material Money 尽可能显示最新货币，并在应用尚未更新时通知用户。

其他应用，如新闻应用，可显示一个简单通知，使用一个 tap-to-update 功能通知用户有更新的内容。 这样做的原因是如果某个用户目前在阅读一篇文章，自动更新将刷新页面，用户将退出他们上次阅读到的地方。

![新闻应用示例，处于正常状态的 Tailpiece](./images/tailpiece-normal.png)![新闻应用示例，准备更新时的 Tailpiece](./images/tailpiece-tap-to-update.png)在线新闻应用 Tailpiece 将自动下载最新新闻，但允许用户手动刷新，以便他们不会退出己看到的文章中的位置。
...
![Tailpiece 使用各种设计小部件通知用户其处于离线状态](./images/tailpiece-offline.png)![Tailpiece 具有一个导航视图，其显示哪些部分可以离线使用。](./images/tailpiece-offline-sidebar.png)如果设备处于离线状态，Tailpiece 将使用一个状态消息通知用户，让用户知道他们仍可以使用此应用。
...
![离线工作的 I/O 应用](./images/io-offline-ready.png)![离线工作的 Chrome Status 网站](./images/chome-offline.png)Google I/O 2016 应用和 Chrome Status 网站在应用可离线工作时通知用户。
...
![下载图标示例就是一个很好的例子](./images/download.png)**宜**：使用可描述此操作的语言和图像。

![服务工作线程图标示例则是个糟糕的例子](./images/service-worker-ready.png)**忌**：避免使用可能让人无法理解的抽象术语。
...
![以下示例使用颜色和文本显示错误，效果很好。](./images/accessibility_color7_do.png)**宜**：使用混合设计元素表达含义

![以下示例仅使用颜色，效果很差。](./images/accessibility_color8_dont.png)**忌**：仅使用颜色描述所进行的操作。
...
![表示离线的各种图标示例](https://developers.google.com/web/fundamentals/instant-and-offline/images/download-icons-exampels.png)
...
![框架布局示例](./images/tailpiece-skel-article.png)![已加载的文章示例](./images/tailpiece-normal_1.png)使用框架布局之前和之后。
